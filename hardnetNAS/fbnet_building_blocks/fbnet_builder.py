...
class Identity(nn.Module):
    def __init__(self, C_in, C_out, stride):
        ...
        if C_in != C_out:
            if stride == 1:
                self.conv = ConvBNRelu(C_in, C_out, kernel=1, stride=stride, pad=0, no_bias=1, use_relu="relu",
                                       bn_type="bn")
            else:
                self.conv = nn.Sequential(
                    nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
                    ConvBNRelu(C_in, C_out, kernel=1, stride=1, pad=0, no_bias=1, use_relu="relu", bn_type="bn")
                )
        else:
            if stride == 1:
                self.conv = None
