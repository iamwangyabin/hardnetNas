...
PRIMITIVES = {
    ...
    "ShuffleV2_A": lambda C_in, C_out, expansion, stride, **kwargs: InvertedResidual(
        C_in, C_out, stride, 1
    ),
    "ShuffleV2_B": lambda C_in, C_out, expansion, stride, **kwargs: InvertedResidual(
        C_in, C_out, stride, 2
    ),
    "MobileV3_RE": lambda C_in, C_out, expansion, stride, **kwargs: MobileBlock(
        C_in, C_out, 3, stride, "RE", False, C_in*3
    ),
    "MobileV3_Hswish": lambda C_in, C_out, expansion, stride, **kwargs: MobileBlock(
        C_in, C_out, 3, stride, "Hswish", False, C_in*3
    ),
    "conv_k3": lambda C_in, C_out, expansion, stride, **kwargs: NormConv(
        C_in, C_out, stride
    ),
    "MaxPooling": lambda C_in, C_out, _,__, **kwargs: DownMaxPool(),
    "Skip": lambda C_in, C_out, expansion, stride, **kwargs: Skip(
        C_in, C_out, stride
    ),
}
    ...
    def forward(self, x):
        if 1 == self.benchmodel:
            if self.stride == 1:
                ...
                if self.inp == self.outp:
                    x1 = x[:, :(x.shape[1] // 2), :, :]
                    x2 = x[:, (x.shape[1] // 2):, :, :]
                    out = self._concat(self.AvgPool(x1), self.banch2(x2))
                else:
                    out = self._concat(self.AvgPool(x), self.banch2(x))
