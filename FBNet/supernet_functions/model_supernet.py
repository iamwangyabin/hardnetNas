...
from FBNet.fbnet_building_blocks.fbnet_builder import ConvBNRelu, Flatten
...
class FBNet_Stochastic_SuperNet(nn.Module):
    def __init__(self, lookup_table, cnt_classes=1000):
        ...
        self.last_stages = nn.Sequential(OrderedDict([
            ("conv_k1", nn.Conv2d(lookup_table.layers_parameters[-1][1], 256, kernel_size = 1)),
            ("avg_pool_k4", nn.AvgPool2d(kernel_size=4)),
            ("flatten", Flatten()),
            ("fc", nn.Linear(in_features=256, out_features=cnt_classes)),
        ]))
