...
class Normalize_0_1(object):
    def __init__(self):
        pass
    def __call__(self, sample):
        sample["im1"] = sample["im1"] / 255
        sample["im2"] = sample["im2"] / 255
        return sample
...
class RandomRotation(object):
    ...
    def __call__(self, sample):
        im1 = sample["im1"]
        im2 = sample["im2"]
        homo12 = sample["homo12"]
        # homo21 = sample["homo21"]
        # angle = random.randint(0, 45)
        angle = 45
        cropimg, w_crop, h_crop, M= self.rotate_bound(im1, angle)

        ## x0 youshang
        ## x1 zuoshang
        ## x2 youxia
        x0,y0 = self.Srotate(math.radians(angle+270), im1.shape[1] / 2-h_crop/2, im1.shape[0] / 2-w_crop/2, im1.shape[1] / 2, im1.shape[0] / 2)
        x1,y1 = self.Srotate(math.radians(angle+270), im1.shape[1] / 2-h_crop/2, im1.shape[0] / 2+w_crop/2 ,im1.shape[1] / 2, im1.shape[0] / 2)
        x2,y2 = self.Srotate(math.radians(angle+270), im1.shape[1] / 2+h_crop/2, im1.shape[0] / 2-w_crop/2 ,im1.shape[1] / 2, im1.shape[0] / 2)
        x3,y3 = self.Srotate(math.radians(angle+270), im1.shape[1] / 2+h_crop/2, im1.shape[0] / 2+w_crop/2 ,im1.shape[1] / 2, im1.shape[0] / 2)
        ...
        src = np.float32([[[0, 0], [0, cropimg.shape[0]], [cropimg.shape[1], 0], [cropimg.shape[1], cropimg.shape[0]]]])
        p_key1 = cv2.perspectiveTransform(np.array([[[x1, y1], [x3, y3], [x0, y0], [x2, y2]]]), homo12)
        p_key2 = np.float32(p_key1) - np.float32([p_key1[0,:,0].min(), p_key1[0,:,1].min()])

        if p_key1.min()<0 or p_key1[0,:,0].max()>im2.shape[1] or  p_key1[0,:,1].max()>im2.shape[0]:
            # if angle%2 ==0 :
            #     angle = 180
            #     cropimg, w_crop, h_crop, M = self.rotate_bound(im1, angle)
            #     x0, y0 = self.Srotate(math.radians(angle + 270), im1.shape[1] / 2 - h_crop / 2,
            #                           im1.shape[0] / 2 - w_crop / 2, im1.shape[1] / 2, im1.shape[0] / 2)
            #     x1, y1 = self.Srotate(math.radians(angle + 270), im1.shape[1] / 2 - h_crop / 2,
            #                           im1.shape[0] / 2 + w_crop / 2, im1.shape[1] / 2, im1.shape[0] / 2)
            #     x2, y2 = self.Srotate(math.radians(angle + 270), im1.shape[1] / 2 + h_crop / 2,
            #                           im1.shape[0] / 2 - w_crop / 2, im1.shape[1] / 2, im1.shape[0] / 2)
            #     x3, y3 = self.Srotate(math.radians(angle + 270), im1.shape[1] / 2 + h_crop / 2,
            #                           im1.shape[0] / 2 + w_crop / 2, im1.shape[1] / 2, im1.shape[0] / 2)
            #
            #     src = np.float32([[[0, 0], [0, cropimg.shape[0]], [cropimg.shape[1], 0], [cropimg.shape[1], cropimg.shape[0]]]])
            #     p_key1 = cv2.perspectiveTransform(np.array([[[x1, y1], [x3, y3], [x0, y0], [x2, y2]]]), homo12)
            #     crop_homo12 = cv2.getPerspectiveTransform(src, np.float32(p_key1))
            #     crop_homo21 = cv2.getPerspectiveTransform(np.float32(p_key1), src)
            #     yy = cv2.warpPerspective(cropimg,crop_homo12,(im2.shape[1],im2.shape[0]))
            #     # from matplotlib import pyplot as plt
            #     # plt.imshow(yy + im2)
            #     # plt.show()
            #     sample["im1"] = cropimg
            #     # sample["im2"] = des
            #     sample["homo12"] = crop_homo12
            #     sample["homo21"] = crop_homo21
            #     # import pdb
            #     # pdb.set_trace()
            #     return sample
            # else:
        ...
        # p_key1 = np.clip(p_key1, 0, 907, out=None)
        # from matplotlib import pyplot as plt
        # plt.imshow(im2)
        # plt.scatter(p_key1[0,0,0], p_key1[0,0,1])
        # plt.scatter(p_key1[0,1,0], p_key1[0,1,1])
        # plt.scatter(p_key1[0,2,0], p_key1[0,2,1])
        # plt.scatter(p_key1[0,3,0], p_key1[0,3,1])
        # plt.show()
        # plt.imshow(im2)
        # plt.scatter(p_key2[0,0,0], p_key2[0,0,1])
        # plt.scatter(p_key2[0,1,0], p_key2[0,1,1])
        # plt.scatter(p_key2[0,2,0], p_key2[0,2,1])
        # plt.scatter(p_key2[0,3,0], p_key2[0,3,1])
        # plt.show()
        # plt.imshow(cropimg)
        # plt.scatter(src[0,0,0], src[0,0,1], c='r')
        # plt.scatter(src[0,1,0], src[0,1,1], c='b')
        # plt.scatter(src[0,2,0], src[0,2,1], c='w')
        # plt.scatter(src[0,3,0], src[0,3,1], c='y')
        # plt.show()
        # crop_homo12 = cv2.getPerspectiveTransform(src, np.float32(p_key1))
            des = im2[int(p_key1[0,:,1].min()):int(p_key1[0,:,1].max()),int(p_key1[0,:,0].min()):int(p_key1[0,:,0].max()),:]
            # p_key1 = cv2.perspectiveTransform(np.array([[[x1, y1], [x3, y3], [x0, y0], [x2, y2]]]), homo12)
            crop_homo12 = cv2.getPerspectiveTransform(src, p_key2)
            crop_homo21 = cv2.getPerspectiveTransform(p_key2, src)
            yy = cv2.warpPerspective(cropimg,crop_homo12,(des.shape[1],des.shape[0]))
            # yy = cv2.warpPerspective(cropimg, crop_homo12, (im2.shape[1], im2.shape[0]))
            # yy = cv2.warpPerspective(des, crop_homo21, (cropimg.shape[1], cropimg.shape[0]))
            sample["im1"] = cropimg
            sample["im2"] = des
            sample["homo12"] = crop_homo12
            sample["homo21"] = crop_homo21
    ...
    def rotate_bound(image, angle):
        ...
        rotate_img = cv2.warpAffine(image, M, (nW, nH))
        len = (nW**2+nH**2)**0.5/2
        if w>h:
            minl = h/2
        else:
        ...
        newW = ratio*nW
        newH = ratio*nH
        rotate_img_crop = rotate_img[int(nH/2-newH/2):int(nH/2+newH/2),int(nW/2-newW/2):int(nW/2+newW/2)]
        # from matplotlib import pyplot as plt
        # plt.imshow(rotate_img_crop)
        return rotate_img_crop,newW,newH,M