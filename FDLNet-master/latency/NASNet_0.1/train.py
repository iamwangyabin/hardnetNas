...
def create_optimizer(
    ...
    for name, param in model.det.named_parameters():
        if param.requires_grad:
            print(name)
    for name, param in model.des.named_parameters():
        if param.requires_grad:
            print(name)
    # import pdb
    # pdb.set_trace()
    ...
    train_data = DataLoader(
        HpatchDataset(
            ...
            transform=transforms.Compose(
                [
                    Grayscale(),
                    Normalize(mean=cfg[cfg.PROJ.TRAIN]["MEAN"], std=cfg[cfg.PROJ.TRAIN]["STD"]),
                    # RandomRotation(),
                    # Normalize_0_1(),
