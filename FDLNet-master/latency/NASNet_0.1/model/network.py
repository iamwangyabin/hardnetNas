...
class Network(nn.Module):
    ...
    def criterion(self, endpoint):
        ...
        # im1_scale = endpoint["im1_scale"]
        # im1_gtscale = endpoint["im1_gtscale"]
        # im2_scale = endpoint["im2_scale"]
        # im2_gtscale = endpoint["im2_gtscale"]
        # im1_orin = endpoint["im1_orin"]
        # im1_gtorin = endpoint["im1_gtorin"]
        # im2_orin = endpoint["im2_orin"]
        # im2_gtorin = endpoint["im2_gtorin"]
        ...
        # geo loss
        #
        # im1_scaleloss = self.det.loss(im1_scale, im1_gtscale, im1_visible)
        # im2_scaleloss = self.det.loss(im2_scale, im2_gtscale, im2_visible)
        # scale_loss = (im1_scaleloss + im2_scaleloss) / 2.0 * 0.001

        #
        # geo loss
        #
        # im1_orinloss = self.det.loss(im1_orin, im1_gtorin, im1_visible)
        # im2_orinloss = self.det.loss(im2_orin, im2_gtorin, im2_visible)
        # orin_loss = (im1_orinloss + im2_orinloss) / 2.0 * 0.001
        ...
        # PLT_SCALAR["scale_loss"] = scale_loss
        # PLT_SCALAR["orin_loss"] = orin_loss
    ...
    def detectAndCompute(self, im_path, device, output_size):
        ...
        # inference
        _, kp, des, scale, angle = self.inference(img, img_info, img_raw)

        return kp, des, img, scale, angle