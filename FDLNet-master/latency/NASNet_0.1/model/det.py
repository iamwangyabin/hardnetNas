...
class RFDet(nn.Module):
    def __init__(
        ...
        for p in self.insnorm_1_1.parameters():
            p.requires_grad = True
        ############################################################################################################
        self.conv_ori_1 = InvertedResidual(16, 16, 1, 1)
        self.insnorm_ori_1 = nn.InstanceNorm2d(16, affine=True)
        self.conv_ori_2 = nn.Conv2d(in_channels=16, out_channels=2, kernel_size=3, stride=1, padding=1,
                                    dilation=dilation)
        self.insnorm_ori_2 = nn.InstanceNorm2d(2, affine=True)
    ...
    def forward(self, photos):
        ...
        ori_map = self.insnorm_ori_1(
            self.conv_ori_1(torch.nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=True)(feature_map)))
        ori_map = L2Norm(torch.nn.Upsample(size=(H, W), mode='bilinear', align_corners=True)(
            self.insnorm_ori_2(self.conv_ori_2(ori_map))), dim=1).permute(0, 2, 3, 1)
