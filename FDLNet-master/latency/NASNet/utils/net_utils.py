...
def pair(
    ...
    left_imS = left_imscale.masked_select(
        left_imtopk.to(torch.bool)
    )  # (B*topk) have grad to detect network
    if left_imorint is not None:
        left_cos, left_sim = left_imorint.squeeze().chunk(chunks=2, dim=-1)
        left_cos = left_cos.masked_select(left_imtopk.to(torch.bool))  # (B*topk)
        left_sim = left_sim.masked_select(left_imtopk.to(torch.bool))  # (B*topk)
