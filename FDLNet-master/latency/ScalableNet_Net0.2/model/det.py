...
class RFDet(nn.Module):
    def __init__(
        ...
        self.features.append(InvertedResidual(16, 16, 1, 2))
        self.features.append(InvertedResidual(16, 16, 1, 1))
        self.features = nn.Sequential(*self.features)
        #----------------------------------------------------------------------------------------------------------#
        self.conv_1_1 = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=ksize, stride=1, padding=padding,
                                  dilation=dilation, )
        self.insnorm_1_1 = nn.InstanceNorm2d(1, affine=True)
        #----------------------------------------------------------------------------------------------------------#
        for p in self.parameters():
            p.requires_grad = frezone

        self.conv_ori_1 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=padding)
        self.insnorm_ori_1 = nn.InstanceNorm2d(16, affine=True)
        self.conv_ori_2 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=padding)
        self.insnorm_ori_2 = nn.InstanceNorm2d(16, affine=True)
        self.conv_ori_3 = nn.Conv2d(in_channels=16, out_channels=2, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, photos):
        ...
        feature_map_2 = torch.nn.Upsample(scale_factor=0.5, mode='bilinear')(feature_map)
        feature_map_3 = torch.nn.Upsample(scale_factor=0.25, mode='bilinear')(feature_map)
