...
class Grayscale(object):
    def __call__(self, sample):
        ...
        :return: sample list
        """
        sample["im1"] = np.expand_dims(color.r(sample["im1"]), -1)
...
class RandomRotation(object):
    ...
    def __call__(self, sample):
        ...
        crop, w_crop,h_crop = self.rotate_image(im1,angle,True)
        x0,y0 = self.Srotate(math.radians(270+angle), im1.shape[1] / 2-h_crop/2, im1.shape[0] / 2-w_crop/2, im1.shape[1] / 2, im1.shape[0] / 2)
        x1,y1 = self.Srotate(math.radians(270+angle), im1.shape[1] / 2-h_crop/2, im1.shape[0] / 2+w_crop/2 ,im1.shape[1] / 2, im1.shape[0] / 2)
        x2,y2 = self.Srotate(math.radians(270+angle), im1.shape[1] / 2+h_crop/2, im1.shape[0] / 2-w_crop/2 ,im1.shape[1] / 2, im1.shape[0] / 2)
        x3,y3 = self.Srotate(math.radians(270+angle), im1.shape[1] / 2+h_crop/2, im1.shape[0] / 2+w_crop/2 ,im1.shape[1] / 2, im1.shape[0] / 2)
        xx = self.rotate(im1,  # 图片
            np.array([x0,y0]), np.array([x1,y1]), np.array([x3,y3]), np.array([x2,y2])
        )
        src = np.float32([[[0, 0], [0, xx.shape[0]], [xx.shape[1], 0], [xx.shape[1], xx.shape[0]]]])
        p_key1 = cv2.perspectiveTransform(np.array([[[x0, y0], [x1, y1], [x2, y2], [x3, y3]]]), homo12)
        ...
        # yy=cv2.warpPerspective(xx,crop_homo12,(im2.shape[1],im2.shape[0]))
    ...
    def rotate_image(img, angle, crop):
        ...
        if crop:
            ...
            key = np.float32([[[x0,y0],[x0 + w_crop,y0],[x0+w_crop,y0+h_crop],[x0,y0+h_crop]]])
            # import pdb
            # pdb.set_trace()
        return img_rotated ,w_crop, h_crop
