...
def select_optimizer(optim, param, lr, wd):
    if optim == "sgd":
        optimizer = torch.optim.SGD(
            filter(lambda p: p.requires_grad, param), lr=lr, momentum=0.9, dampening=0.9, weight_decay=wd
        )
    elif optim == "adam":
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, param), lr=lr, weight_decay=wd)
    else:
        ...
        for i_batch, sample_batched in enumerate(train_data, 1):
            ...
            with autograd.detect_anomaly():
                for des_train in range(0, cfg.TRAIN.DES):
                    model.zero_grad()
                    des_optim.zero_grad()
                    # endpoint = checkpoint_sequential(model,3,batch)
